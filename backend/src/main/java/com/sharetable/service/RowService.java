...
public class RowService {
    ...
    public RowService(TableRepository tableRepository, RowRepository rowRepository, TableUpdateBroadcaster broadcaster) {
    ...
    public Optional<Row> addRow(UUID shareToken) {
        return tableRepository.findByShareTokenWithColumns(shareToken)
            .map(table -> {
                int maxOrder = table.getRows().stream()
                    .mapToInt(Row::getOrder)
                    .max()
                    .orElse(-1);
